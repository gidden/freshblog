!(function() {
    ...
    var outputResults = function(results, el, style) {
	if (typeof(style)==='undefined') style = 'title';
	...
	results.forEach(function(result) {
	    ...
	    // change this to change the search heading
	    if (style == 'date') {
		link.innerHTML = result.date.split(' ')[0];
	    } else if (style == 'title') {
		link.innerHTML = result.title;
	    } else {
	        throw new Error("Unknown search link style.");
	    }
	...
	outputResults(this.results, this.el, options.style);
